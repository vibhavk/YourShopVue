<template>
    <tr>
        ...
        <td class="text-right">
            {{ line.product.price*69 | currency }}
        </td>
        <td class="text-right">
            {{ (line.quantity * line.product.price*69) | currency }}
        </td>
        ...
        methods: {
            sendChangeEvent($event) {
                if ($event.target.value > 0) {
                    this.$emit("quantity", Number($event.target.value)); //sending an event to parent component
