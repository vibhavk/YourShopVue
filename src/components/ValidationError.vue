...
export default {
    ...
    computed: {
        show() {
            return this.validation.$dirty && this.validation.$invalid //these $dirty and all can be accesed because this child (ValidationError component) was passed $v.name as validation prop by its parent
        },
        messages() {
            let messages = [];
